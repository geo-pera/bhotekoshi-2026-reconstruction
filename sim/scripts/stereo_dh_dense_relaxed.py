...
tp = extract_tie_points(A, B, gtA, gtB, grid_res=80, window_size=128,
                        max_shift=120, min_reliability=12.0, nodata=0,
                        ransac=False, min_std=1.5, min_range_fraction=0.002)
n = len(tp["x_map"])
...
with open(f"{ROOT}/sim/inputs/stereo_dh_dense_relaxed.csv", "w", newline="") as f:
